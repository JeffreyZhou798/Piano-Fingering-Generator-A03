...
function addFingeringToPart(
  ...
  let notesSkipped = 0;
  let tieSkipped = 0;
  let mismatchDetails: Array<{index: number, expected: number, got: number}> = [];
    ...
    for (const note of Array.from(notes)) {
      ...
      if (fingeringIndex < fingerings.length) {
        ...
        if (Math.abs(fingering.pitch - midiPitch) <= 1) {
          ...
          let found = false;
          const lookAhead = Math.min(5, fingerings.length - fingeringIndex);
          ...
          for (let offset = 1; offset < lookAhead; offset++) {
            ...
            if (Math.abs(nextFingering.pitch - midiPitch) <= 1) {
              ...
              // 将已使用的指法标记（通过交换到当前位置）
              fingerings[fingeringIndex + offset] = fingering;
              fingeringIndex++;
              break;
            }
          }
          ...
          if (!found) {
            ...
            if (mismatchDetails.length < 10) {
              mismatchDetails.push({
                ...
                expected: fingering.pitch,
                got: midiPitch
              });
            }
            console.warn(`Pitch mismatch at index ${fingeringIndex}: expected ${fingering.pitch}, got ${midiPitch}`);
            fingeringIndex++; // 跳过这个指法
          }
        }
      }
    }
  ...
  if (fingeringsAdded < fingerings.length) {
    console.warn(`${hand} hand: ${fingerings.length - fingeringsAdded} fingerings were not added (total ${fingerings.length}, added ${fingeringsAdded})`);
  }
  
  if (mismatchDetails.length > 0) {
    console.warn(`${hand} hand: First ${mismatchDetails.length} mismatches:`, mismatchDetails);
  }
}
